...
class UserTest < ActiveSupport::TestCase
	...
	test "a user should have a profile name without spaces" do
		user = User.new(first_name: 'Chris', last_name: 'Callahan', email: 'user2@example.com')
	...
	test "a user can have a correctly formatted profile name" do
		...
		user.profile_name = 'chriscallahan1'
		assert user.valid?
	end
	...
	test "that calling to_param on a user returns the profile_name" do
		assert_equal 'chriscallahan', users(:chris).to_param
	end

	context "#has_blocked?" do
		should "return true if a user has blocked another user" do
			assert users(:chris).has_blocked?(users(:blocked_friend))
		end

		should "return false if a user has not blocked another user" do
			assert !users(:chris).has_blocked?(users(:yankai))
		end
	end
