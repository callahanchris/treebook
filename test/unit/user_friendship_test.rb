...
class UserFriendshipTest < ActiveSupport::TestCase
	...
	context "#mutual_friendship" do
		setup do
			UserFriendship.request users(:chris), users(:yankai)
			@friendship1 = users(:chris).user_friendships.where(friend_id: users(:yankai).id).first
			@friendship2 = users(:yankai).user_friendships.where(friend_id: users(:chris).id).first
		end
	...
	context "#block!" do
		...
		should "not allow new requests once blocked" do
			@user_friendship.block!
			uf = UserFriendship.request users(:chris), users(:mike)
			assert !uf.save
		end
	end
end
